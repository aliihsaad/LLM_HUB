...
function resolveAgentPath(value, cwd) {
...
function listAgentFiles(target, cwd, max = 200) {
    const root = resolveAgentPath(target || '.', cwd);
    if (!fs.existsSync(root))
        return `Path not found: ${path.relative(cwd, root)}`;
...
function readAgentFile(filePath, cwd, startLine = 1, maxLines = 240) {
    const resolved = resolveAgentPath(filePath, cwd);
    if (!fs.existsSync(resolved))
        return `File not found: ${filePath}`;
    if (!fs.statSync(resolved).isFile())
        return `Not a file: ${filePath}`;
    if (!allowedTextFile(resolved))
        return `Refusing to read non-text file: ${filePath}`;
...
function searchAgentFiles(query, target, cwd, max = 80) {
    if (!query)
...
function summarizeDiff(before, after, fileLabel) {
    if (before === after)
        return `No changes in ${fileLabel}.`;
...
async function confirmAction(rl, label, details, assumedYes) {
    if (assumedYes)
        return true;
    ...
    for (const candidate of candidates) {
        try {
            ...
            if (parsed && typeof parsed === 'object' && typeof parsed.tool === 'string') {
                return { tool: parsed.tool, reason: parsed.reason, args: parsed.args ?? {} };
            }
        }
        catch { }
    }
...
function formatToolResult(tool, ok, outputText) {
...
async function runAgentTool(call, options, rl) {
    ...
    try {
        switch (call.tool) {
            ...
            case 'read_file': {
                return formatToolResult(call.tool, true, readAgentFile(String(args.path ?? ''), cwd, Number(args.startLine ?? args.start_line ?? 1), Number(args.maxLines ?? args.max_lines ?? 240)));
            }
            case 'search': {
                return formatToolResult(call.tool, true, searchAgentFiles(String(args.query ?? ''), typeof args.path === 'string' ? args.path : '.', cwd, Number(args.max ?? 80)));
            }
            case 'replace_in_file': {
                ...
                if (!before.includes(search))
                    throw new Error('Exact search text not found');
                ...
                if (!ok)
                    return formatToolResult(call.tool, false, 'User rejected edit.');
            ...
            case 'write_file': {
                ...
                if (!ok)
                    return formatToolResult(call.tool, false, 'User rejected write.');
            ...
            case 'run_command': {
                ...
                if (!ok)
                    return formatToolResult(call.tool, false, 'User rejected command.');
...
async function runAgentTurn(task, options, initialContext = '') {
...
async function runInteractiveAgent(options, initialContext = '') {
    const rl = readline.createInterface({ input, output });
    ...
    try {
        while (true) {
            ...
            if (task === '/models') {
                const cliOptions = program.opts();
                const data = await apiRequest('/v1/models', cliOptions);
                console.table(data.data.slice(0, 30).map(item => ({ model: item.id, provider: item.owned_by ?? '' })));
                continue;
            }
