...
function migrateModelsV13(db: Database.Database) {
  ...
  const apply = db.transaction(() => {
    for (const model of starterModels) {
      insert.run(...model);
      const row = getModel.get(model[0], model[1]) as { id: number } | undefined;
      if (row) addFallback.run(row.id, priority++);
    }
  });
  apply();
}
...
function migrateModelsV14(db: Database.Database) {
  ...
  const currentGoogleFreeModels: Array<[
    string,
    string,
    string,
    number,
  ...
  const apply = db.transaction(() => {
    ...
    let priority = maxPriority + 1;
    for (const model of currentGoogleFreeModels) {
      const row = getModel.get(model[0], model[1]) as { id: number; enabled: number } | undefined;
      if (row) addFallback.run(row.id, priority++, row.enabled);
    }
...
function migrateModelsV15(db: Database.Database) {
  ...
  const bazaarlinkModels: Array<[
    string, string, string, number, number, string,
    number | null, number | null, number | null, number | null, string, number | null, number,
  ]> = [
  ...
  const apply = db.transaction(() => {
    for (const model of bazaarlinkModels) insert.run(...model);
...
function migrateModelsV17(db: Database.Database) {
  ...
  const newFreeModels: Array<[
    string, string, string, number, number, string,
    number | null, number | null, number | null, number | null, string, number | null, number, number,
  ]> = [
  ...
  const apply = db.transaction(() => {
    for (const model of newFreeModels) insert.run(...model);
...
function migrateModelsV21(db: Database.Database) {
  ...
  const disable = db.prepare(
    "UPDATE models SET enabled = 0 WHERE platform = 'nvidia' AND model_id = ?",
  );

  const apply = db.transaction(() => {
    for (const modelId of retired) disable.run(modelId);
    for (const a of additions) insert.run(...a);
 ...
 */
function retireDeadCatalogRowsV22(db: Database.Database) {
  ...
  const alreadyApplied = db
    ...
    .get(APPLIED_KEY) as { ok: number } | undefined;
  if (alreadyApplied) return;

  const apply = db.transaction(() => {
    for (const platform of disableProviders) disableAllFor.run(platform);
    for (const modelId of retiredOpenRouter) disableRow.run('openrouter', modelId);
    for (const modelId of retiredLlm7) disableRow.run('llm7', modelId);
 ...
 * Idempotent.
 */
function flagPaidGoogleModels(db: Database.Database) {
