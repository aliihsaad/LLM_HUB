 ...
 */
export function categorizeModel(
  modelId: string,
  displayName: string,
  intelligenceRank: number,
  speedRank: number,
  sizeLabel: string,
): { category: ModelCategory; specializations: string[] } {
  ...
  // Fast models
  if (
    /flash|instant|nano|lite|small|8b|1\.2b|9b/.test(id + name) ||
  ...
  if (
    (intelligenceRank <= 5 || /pro|frontier|maverick|k2|gpt-5/.test(id + name)) &&
 ...
 */
export function autoCategorizeAllModels(): void {
  const db = getDb();
  ...
  const tx = db.transaction(() => {
    for (const m of models) {
      const { category, specializations } = categorizeModel(
        m.model_id,
        m.display_name,
        m.intelligence_rank,
        m.speed_rank,
...
export function getModelsByCategory(
  ...
  const rows = category
    ? db.prepare(sql).all(category) as any[]
...
export function getCategoryStats(): Array<{
  ...
  const rows = db.prepare(`
    ...
    GROUP BY m.category
    ORDER BY count DESC
  `).all() as Array<{ category: string; count: number; free_count: number }>;
