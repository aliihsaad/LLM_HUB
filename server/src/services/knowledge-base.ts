...
const CONTEXT7_TOPICS: Record<string, string> = {
 ...
 */
export function logContext7Warning(): void {
  if (context7WarningLogged) return;
 ...
 */
export function initKnowledgeBase(db: ReturnType<typeof getDb>): void {
  db.exec(`
 ...
 */
export async function fetchFromContext7(provider: string, topic: string): Promise<string | null> {
  const config = getContext7Config();
  ...
  const library = await resolveContext7Library(
    providerMetadata?.displayName ?? provider,
    providerMetadata?.docsUrl,
    config,
  );
...
function getContext7Headers(config: Context7Config): Record<string, string> {
  const headers: Record<string, string> = { Accept: 'application/json' };
  if (config.apiKey) headers.Authorization = `Bearer ${config.apiKey}`;
  return headers;
}

async function fetchContext7Json<T>(url: URL, config: Context7Config): Promise<T> {
...
async function resolveContext7Library(
...
): Promise<Context7LibraryMatch | null> {
  ...
  const data = await fetchContext7Json<Context7SearchResponse>(url, config);
  return data.results?.[0] ?? null;
}

async function fetchContext7Topic(
  libraryId: string,
  query: string,
  config: Context7Config,
): Promise<string | null> {
  ...
  const data = await fetchContext7Json<Context7ContextResponse>(url, config);
  return formatContext7Context(data);
}

function formatContext7Context(data: Context7ContextResponse): string | null {
 ...
 */
export async function syncContext7Knowledge(): Promise<KnowledgeSyncResult> {
  const config = getContext7Config();
  if (!config.configured) {
    logContext7Warning();
  ...
  let synced = 0;
  let failed = 0;
  const providerResults: KnowledgeSyncProviderResult[] = [];
 ...
 */
export function getKnowledgeSyncStatus(): KnowledgeProviderStatus[] {
  const db = getDb();
 ...
 */
export function queryKnowledge(query: Context7Query): KnowledgeEntry[] {
 ...
 */
export function getProviderKnowledge(provider: string): KnowledgeEntry[] {
  const db = getDb();
  const rows = db.prepare(`
    SELECT id, provider, topic, content, source_url, version, fetched_at, last_verified_at
    FROM knowledge_base
 ...
 */
export function searchAllKnowledge(query: string, limit = 10): KnowledgeEntry[] {
