...
function buildWhere(req) {
    ...
    const params = [getSinceTimestamp(range)];
    if (status !== 'all') {
        clauses.push('status = ?');
        params.push(status);
    }
...
logsRouter.get('/', (req, res) => {
  ...
  `).all();
    ...
    const penaltyByPlatform = new Map();
    for (const penalty of getAllPenalties()) {
        const row = db.prepare('SELECT platform FROM models WHERE id = ?').get(penalty.modelDbId);
        if (!row)
            continue;
        penaltyByPlatform.set(row.platform, (penaltyByPlatform.get(row.platform) ?? 0) + penalty.penalty);
    }
    const flagMap = new Map();
    const topFlagByPlatform = new Map();
    ...
    const rankings = aggregateRows.map((row) => {
        ...
        const score = Math.max(0, round1(successRate * 0.55
            + latencyScore * 0.20
            + keyHealthScore * 0.15
            + volumeScore * 0.10
