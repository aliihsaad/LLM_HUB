...
const CATALOG_SYNC_PLATFORMS = new Set([
    'huggingface',
    'vercel',
    'modelscope',
    'qwen',
...
export function isBazaarlinkFreeChatEntry(entry) {
 ...
 */
export function isGoneMessage(message) {
    if (!message)
        return false;
 ...
 */
export function recordGoneStreak(db, modelDbId, isGone) {
    if (!isGone) {
    ...
    WHERE a.model_db_id = ?
  `).get(modelDbId);
    if (!row || row.enabled !== 1 || row.gone_streak < GONE_STREAK_TO_RETIRE)
        return null;
    db.prepare('UPDATE models SET enabled = 0 WHERE id = ?').run(modelDbId);
    console.warn(`[ModelScout] Retired ${row.platform}/${row.model_id} — gone on ${row.gone_streak} consecutive checks`);
    return row.model_id;
}
/**
 ...
 */
export async function checkModelAvailability(modelDbId) {
    const db = getDb();
  ...
  `).get(modelDbId);
    ...
    try {
        ...
        const probeMessages = [{ role: 'user', content: 'hi' }];
 ...
 */
export async function scoutAllModels(delayMs = 2000) {
    const db = getDb();
    ...
    `).get(platform);
        ...
        try {
            ...
            if (platform === 'bazaarlink') {
                applyPricingDrift(db, platform, (data.data ?? []));
            }
            let discoveredForPlatform = 0;
            for (const entry of data.data ?? []) {
                if (!entry.id)
                    continue;
                if (knownSet.has(entry.id))
                    continue;
                ...
                if (platform === 'bazaarlink' && !isBazaarlinkFreeChatEntry(entry))
                    continue;
 ...
 */
export async function discoverAndPersistNewModels() {
    const db = getDb();
  ...
  `);
    const row = db.prepare(`SELECT COALESCE(MAX(priority), 0) AS priority FROM fallback_config`).get();
    ...
    const tx = db.transaction(() => {
        for (const candidate of discovered) {
            const existing = getModelId.get(candidate.platform, candidate.modelId);
            if (existing) {
            ...
            const result = insertModel.run(candidate.platform, candidate.modelId, candidate.displayName, intelligenceRank, speedRank, 'Large', null, null, null, null, '~? (discovered)', null, enabled, candidate.isFree === false ? 0 : 1);
            if (result.changes !== 1) {
                skippedKnown.push(candidate);
                continue;
            }
 ...
 */
export function applyPricingDrift(db, platform, entries) {
    const byId = new Map(entries.filter(e => e.id).map(e => [e.id, e]));
    const rows = db.prepare('SELECT model_id, is_free, enabled FROM models WHERE platform = ?').all(platform);
    const becamePaid = [];
    const wentMissing = [];
    ...
    tx();
    if (becamePaid.length)
        console.warn(`[ModelScout] ${platform}: pricing drift, now paid: ${becamePaid.join(', ')}`);
    if (wentMissing.length)
        console.warn(`[ModelScout] ${platform}: missing upstream, disabled: ${wentMissing.join(', ')}`);
    return { becamePaid, wentMissing };
}
// — Background scheduler —
let scoutIntervalId = null;
export function startModelScout() {
    if (scoutIntervalId)
        return;
...
export function stopModelScout() {
    if (scoutIntervalId) {
